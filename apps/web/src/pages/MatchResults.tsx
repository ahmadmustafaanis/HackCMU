...
export default function MatchResults() {
  ...
  const created = result?.eventType === "CREATED";
  ...
  return (
    <div className="flex flex-1 flex-col overflow-y-auto p-5">
      ...
      <h1 className="font-display text-xl font-medium text-ink">
        {created ? "You started a new event" : "Your matched activity"}
      </h1>
      <p className="mt-1 text-sm text-muted">
        {created
          ? "Nobody else was doing this yet, so we created a new activity with you as the host."
          : "Review the event before joining."}
      </p>
      ...
      {loadState === "ready" && activity && created && (
        <article className="mt-5 overflow-hidden rounded-2xl border-2 border-primary/40 bg-card shadow-sm">
          <div className="flex items-center gap-2 bg-primary px-5 py-3 text-white">
            <SparkIcon className="h-5 w-5 shrink-0" />
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-white/80">New event</p>
              <p className="text-sm font-semibold">You're hosting this one</p>
            </div>
          </div>
          <div className="p-5">
            <h2 className="font-display text-lg font-medium text-ink">{activity.title}</h2>
            ...
            <div className="mt-4 flex flex-wrap gap-3 text-sm text-muted">
              ...
              <span>You · 1 going</span>
            </div>
            <p className="mt-4 rounded-xl bg-primary/5 px-3 py-2 text-sm text-ink">
              Other students can find and join this from Discover. It stays up until {activity.timeLabel.split(" – ")[0]}.
            </p>
            <button
              type="button"
              onClick={() => navigate("/activities")}
              className="pressable mt-5 w-full rounded-2xl bg-primary py-3 text-sm font-semibold text-white"
            >
              View my new activity
            </button>
          </div>
        </article>
      )}
      {loadState === "ready" && activity && !created && (
        <article className="mt-5 rounded-2xl border border-line bg-card p-5 shadow-sm">
          ...
          <h2 className="mt-2 text-lg font-semibold text-ink">{activity.title}</h2>
          <p className="mt-1 text-sm text-muted">{activity.description}</p>
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-muted">
            <span>{activity.approximateLocation}</span>
            <span>{activity.timeLabel}</span>
          ...
          {lastIntent && (
            <button type="button" onClick={() => navigate("/matching", { state: { ...lastIntent, forceCreate: true } })} className="mt-3 w-full rounded-2xl border border-line py-3 text-sm font-semibold text-ink">Create my own activity</button>
          )}
        </article>
      )}
